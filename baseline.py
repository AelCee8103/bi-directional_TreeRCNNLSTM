    ...
    def forward(self, tree, leaf_features):
        ...
        def process_node(node):
            if node.is_leaf:
                ...
                feat = leaf_features[node.index]  # (batch, feature_dim)
                h, c = self.cell(feat, [], [])
                node.hidden = h
                node.cell = c
                return h, c
...
class TreeStructuredRCNNLSTM(nn.Module):
    ...
    def forward(self, trees, leaf_indices, leaf_features):
        """
        Args:
            trees: list of TreeNode objects (batch)
            leaf_indices: list of lists, word indices for each tree's leaves
            leaf_features: CNN features for each leaf
        Returns:
            vad_pred: (batch, 3) predicted V, A, D values
        """
        batch_size = len(trees)

        # Process each tree and get root hidden states
        root_hiddens = []
        for i, tree in enumerate(trees):
            # Get CNN features for this tree's leaves
            tree_leaf_features = [leaf_features[j] for j in leaf_indices[i]]
            root_h = self.tree_lstm(tree, tree_leaf_features)
            root_hiddens.append(root_h)

        # Stack root hidden states
        root_hidden = torch.stack(root_hiddens, dim=0)  # (batch, hidden_size)
        root_hidden = self.dropout(root_hidden)

        # Predict VAD
        vad_pred = self.fc(root_hidden)  # (batch, 3)
